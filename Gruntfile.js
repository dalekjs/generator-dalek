...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    src: {
      lint: ['Gruntfile.js', 'app/*.js', 'tests/*.js'],
      complexity: ['app/*.js'],
      test: ['tests/*.js'],
      src: ['app/*.js']
    },
    ...
    documantix: {
      options: {
        ...
        target: 'report/docs',
        vars: {
          title: 'DalekJS - Documentation - Yeoman',
          desc: 'DalekJS - Documentation - Yeoman',
          docs: true
        }
      },
      src: ['app/index.js']
  ...
  grunt.registerTask('preparePlato', function () {
    ...
    var platoDummyFolders = ['report', 'report/coverage', 'report/complexity', 'report/complexity/files', 'report/complexity/files/tests', 'report/complexity/files/app/', 'report/complexity/files/app/index_js'];
    var platoDummyFiles = ['/report/complexity/report.history.json', '/report/complexity/files/report.history.json', '/report/complexity/files/app/report.history.json', '/report/complexity/files/app/index_js/report.history.json'];
  ...
  grunt.registerTask('prepareCoverage', function () {
    ...
    // generate folders
    ['coverage', 'coverage/app', 'report', 'report/coverage', 'report/coverage'].forEach(function (folder) {
    ...
    var coverageHelper = 'require("blanket")({pattern: [require("fs").realpathSync(__dirname + "/../app/index.js")]});';
  ...
  grunt.registerTask('archive', function () {
    ...
    grunt.util.spawn({cmd: 'git', args: ['describe', '--abbrev=0', '--tags']}, function (error, result) {
      ...
      if (grunt.file.isFile('_raw/docs/' + lastTag + '/yeoman.html')) {
      ...
      grunt.file.copy('report/docs/yeoman.html', '_raw/docs/' + lastTag + '/yeoman.html');
